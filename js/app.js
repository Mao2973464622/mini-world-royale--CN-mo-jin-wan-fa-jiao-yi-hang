...
(function () {
  ...
  $("#chips").addEventListener("click", (e) => {
    ...
    if (["up", "down", "flat", "fav"].includes(state.filter)) state.cat = ALL_CAT;
  ...
  async function boot() {
    ...
    SNAPSHOTS.forEach((s) => {
      ...
      if (miss.length) console.warn(`[数据] 快照 ${s.id} 缺 ${miss.length} 项价格：`, miss.map(keyOf));
    });
